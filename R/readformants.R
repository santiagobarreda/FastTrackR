...
#' 
#' This function returns objects of the class 'formants' that contain all information about all analyses for all files, in addition to some other information. 'formants' objects are lists of lists of dataframes. At the top level is the list containing all information for all files. This list has n elements for n files. Each list element is a list of x dataframe, for an analysis with x different analysis steps. Each individual analysis is stored in a  data frame containing formant frequencies and bandwidths for the first 3-4 formants. These values are rounded, which makes display easier and makes the resulting object much smaller in memory (and on your hard drive). 
#' 
...
#' 
#' Every data frame representing a single Praat formant object needs to have some information associated with it, and this is done using attributes for the object. The attributes for every dataframe representing a formant object include: timestep (the analysis time step, in ms), w1 (the location of the first analysis window, in ms), maxformant (the maximum formant frequency, in Hz), and filename, containing the label used for the formant object (relating to the wav file filename). 
#' 
#' Both the toplevel list and the lists interior to the 'formants' object have attributes that contain information that might be useful to the user, such as: analysis cutoffs, sound labels, number of files, number of analysis levels, time step, duration, and more.  
#'
